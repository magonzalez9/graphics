...
Helicopter.prototype.drawTail = function (){
    ...
    gl.uniformMatrix4fv(uModel_view, false, flatten(stack.top())); // set view transform
    gl.uniform4fv(uColor, vec4(1, 0, .1, .9)); // set color to red
    ...
    stack.multiply(translate(0,.3,2.05));
     gl.uniform4fv(uColor, vec4(.3, 1, .1, .3));  // set color to red
    ...
    gl.uniform4fv(uColor, vec4(1,0,0.1,.3));  // set color to red
    gl.uniformMatrix4fv(uModel_view, false, flatten(stack.top())); // set view transform
